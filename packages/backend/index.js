...
(function ($) {
		...
		media_uploader.on('insert', function () {
			...
			switch (event.target.id) {
				case 'pwb_brand_image_select':
					current_selector =
						'.taxonomy-pwb-brand #pwb_brand_' + 'image';
					break;
				case 'pwb_brand_banner_select':
					current_selector =
						'.taxonomy-pwb-brand #pwb_brand_' + 'banner';
					break;
			}
			...
			if ($('.pwb_brand_image_selected', $imageSelectorScope).length) {
				$('.pwb_brand_image_selected span', $imageSelectorScope).html(
					image_html
				);
			} else {
				$imageSelectorScope.append(
					'<div class="pwb_brand_image_selected"><span>' +
						image_html +
						'</span></div>'
				);
			}
	...
	$(
		'.taxonomy-pwb-brand #pwb_brand_image_select, .taxonomy-pwb-brand #pwb_brand_banner_select'
	...
	if (
		...
		$('body').hasClass('taxonomy-pwb-brand')
	) {
		$(document).ajaxSuccess(function (event, xhr, settings) {
			...
			if (
				...
				~settings.data.indexOf('taxonomy=pwb-brand')
			) {
				$('#pwb_brand_image').val('');
	...
	function add_delete_link($imageSelectorScope) {
		$('.pwb_brand_image_selected span', $imageSelectorScope).append(
			'<a href="#" class="pwb_brand_image_selected_remove">X</a>'
		);

		$('.pwb_brand_image_selected_remove', $imageSelectorScope).on(
			'click',
			function (event) {
	...
	/* ····························· Edit brands page ····························· */
	$('.taxonomy-pwb-brand table .column-featured > span')
		.not('pwb-blocked')
		.on('click', function (e) {
			e.preventDefault();
			...
			$.post(pwb_ajax_object_admin.ajax_url, data, function (response) {
				...
				if (response.success) {
					var $featuredCount = $(
						'.taxonomy-pwb-brand .pwb-featured-count > span'
					);
					if (response.data.direction == 'up') {
						$featuredCount.html(
							parseInt($featuredCount.text()) + 1
						);
					} else {
						$featuredCount.html(
							parseInt($featuredCount.text()) - 1
	...
	$('.taxonomy-pwb-brand #pwb-first-featured-brands').on(
		'change',
		function (e) {
			e.preventDefault();
			$('#screen-options-apply').replaceWith(
				'<img src="' +
					pwb_ajax_object_admin.site_url +
			...
			var data = {
				...
				nonce: pwb_ajax_object_admin.nonce
					.pwb_admin_save_screen_settings,
			};
			$.post(pwb_ajax_object_admin.ajax_url, data, function (response) {
				location.reload();
			});
	...
	$('#wc_pwb_admin_tab_tools_migrate').on('change', function () {
		if ($(this).val() != '-') {
			if (confirm(pwb_ajax_object_admin.translations.migrate_notice)) {
				$('html').append(
					'<div class="pwb-modal"><div class="pwb-modal-inner"></div></div>'
				);
				$('.pwb-modal-inner').html(
					'<p>' +
						pwb_ajax_object_admin.translations.migrating +
						'</p>'
				);
				...
				$.post(
					pwb_ajax_object_admin.ajax_url,
					data,
					function (response) {
						setTimeout(function () {
	...
	$('#wc_pwb_admin_tab_tools_dummy_data').on('change', function () {
		if ($(this).val() != '-') {
			if (confirm(pwb_ajax_object_admin.translations.dummy_data_notice)) {
				...
				$('.pwb-modal-inner').html(
					'<p>' +
						pwb_ajax_object_admin.translations.dummy_data +
						'</p>'
				);
	...
	var $systemStatusBtn = $('#wc_pwb_admin_tab_tools_system_status').siblings(
		'p'
	);
	...
	$('.wc_pwb_admin_tab_status_btn').on('click', function (e) {
		...
		if (!$('#wc_pwb_admin_status_result').length) {
			var $systemStatusTextarea = $(
				'#wc_pwb_admin_tab_tools_system_status'
			);
			$('<pre id="wc_pwb_admin_status_result"></pre>').insertAfter(
		...
		$('#wc_pwb_admin_status_result').html(
			'<img src="' +
				pwb_ajax_object_admin.site_url +
	...
	$(document).on(
		'click',
		'.pwb-notice-dismissible .notice-dismiss',
		function (e) {
			e.preventDefault();

			var noticeName = $(this)
				.closest('.pwb-notice-dismissible')
				.data('notice');
