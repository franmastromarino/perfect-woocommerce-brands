...
jQuery(function ($) {
	...
	if (typeof $.fn.slick === 'function') {
		$('.pwb-carousel').slick({
			...
			draggable: false,
			prevArrow:
				'<div class="slick-prev"><span>' +
				pwb_ajax_object.carousel_prev +
				'</span></div>',
			nextArrow:
				'<div class="slick-next"><span>' +
				pwb_ajax_object.carousel_next +
				'</span></div>',
			speed: 300,
	...
	$('.pwb-filter-products.pwb-hide-submit-btn input').on(
		'change',
		function () {
			PWBFilterByBrand();
		}
	...
	if (brands != null) {
		...
		for (var i = 0, l = brands_array.length; i < l; i++) {
			$('.pwb-filter-products input[type="checkbox"]').each(function (
				index
			) {
				if ($(this).val()) {
